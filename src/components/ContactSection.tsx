...
export function ContactSection() {
  ...
  const contactMethods = [
    { 
      name: 'Email', 
      icon: Mail, 
      url: 'mailto:falecom@tauatech.com', 
      label: 'falecom@tauatech.com' 
    },
    { 
      name: 'WhatsApp', 
      icon: MessageCircle, 
      url: 'https://wa.me/5561982009880', 
      label: '+55 61 98200-9880' 
    },
    { 
      name: 'GitHub', 
      icon: Github, 
      url: 'https://github.com', 
      label: 'github.com/tauatech' 
    },
    { 
      name: 'LinkedIn', 
      icon: Linkedin, 
      url: 'https://linkedin.com', 
      label: 'linkedin.com/tauatech' 
    },
  ]

  const LogoSVG = ({ className }: { className?: string }) => (
    <svg viewBox="0 0 398 191" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
  ...
  return (
    <section id="contact" className="min-h-screen flex items-center py-24 relative bg-white text-black font-['Anonymous_Pro']">
      <div className="container mx-auto px-6 md:px-12">
        <div className="max-w-7xl mx-auto">
          ...
          >
            <h2 className="text-5xl md:text-7xl lg:text-[100px] leading-none tracking-tighter cursor-default transition-all duration-500 inline-block font-[900] uppercase">
              {t.contact.title.split('').map((char, i) => (
                <span key={i} className="hover:text-[#FF0000] transition-colors duration-300">{char}</span>
              ))}
              <br />
              {t.contact.highlight.split('').map((char, i) => (
                <span key={i} className="text-[#FF0000] hover:text-black transition-colors duration-300">{char}</span>
              ))}
              <span className="text-[#FF0000] hover:text-black transition-colors duration-300">.</span>
            </h2>
            <p className="text-xl md:text-2xl text-black/60 mt-8 max-w-2xl uppercase tracking-wider">
              {t.contact.subtitle}
            </p>
          </motion.div>

          {/* Clean Contact Grid - Black Background & Enhanced Vector Icons */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-0 md:divide-x md:divide-[#FF0000]/20 bg-black">
            {contactMethods.map((item, index) => {
              ...
              return (
                <motion.a
                  ...
                  viewport={{ once: true }}
                  className="group flex flex-col items-center justify-center gap-8 p-12 transition-all duration-500 hover:bg-[#FF0000] text-white"
                >
                  <div className="w-14 h-14 md:w-20 md:h-20 flex items-center justify-center relative">
                    <Icon className="w-full h-full stroke-[1px] group-hover:scale-110 transition-transform duration-500" />
                  </div>
                  
                  <div className="flex flex-col items-center gap-3 w-full">
                    <div className="flex items-center gap-2">
                      <span className="text-[8px] font-bold tracking-[0.5em] uppercase text-[#FF0000] group-hover:text-white/70 transition-colors duration-500">
                        {item.name}
                      </span>
                      <ArrowUpRight className="w-2.5 h-2.5 text-white/20 group-hover:text-white group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-300" />
                    </div>
                    <span className="text-[9px] md:text-[10px] font-light tracking-[0.3em] break-all uppercase text-white/40 group-hover:text-white transition-colors duration-500">
                      {item.label}
                    </span>
                  </div>
                </motion.a>
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            className="mt-24 border-t border-black/10 pt-8 flex flex-col md:flex-row justify-between items-center gap-6 text-sm text-black/40 uppercase tracking-widest"
          >
      ...
      <div className="absolute top-1/2 left-0 -translate-y-1/2 -rotate-90 origin-left pointer-events-none opacity-[0.03] select-none">
        <span className="text-[20vh] font-black whitespace-nowrap leading-none uppercase tracking-tighter">
