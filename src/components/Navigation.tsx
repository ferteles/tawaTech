...
export function Navigation() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const scrollPosition = window.scrollY + 100 // Threshold for topbar
    ...
    >
      <div className="w-full px-6 md:px-12 flex items-center justify-between gap-4 md:gap-8 lg:gap-12">
        ...
        <div className="flex-shrink-0">
          ...
          >
            <LogoSVG className="h-7 md:h-10 w-auto" />
          </button>
        </div>

        {/* Desktop Nav - Better responsive gaps */}
        <nav className="hidden lg:flex items-center gap-6 xl:gap-12 flex-1 justify-center">
          {navItems.map((item) => (
            <button
              key={item.id}
            ...
            >
              <span
                className={`text-[14px] xl:text-lg tracking-[0.2em] xl:tracking-[0.3em] uppercase transition-colors duration-300 font-bold whitespace-nowrap ${
                  activeSection === item.id ? 'text-[#FF0000]' : 'text-white/60 hover:text-white'
                }`}
              >
                {item.label}
              </span>
              {activeSection === item.id && (
                <motion.div
                  ...
                  className="absolute -bottom-1 left-0 w-full h-0.5 bg-[#FF0000]"
        ...
        {/* Desktop Nav for Medium screens (Tablet) - Fewer items or smaller gap */}
        <nav className="hidden md:flex lg:hidden items-center gap-4 flex-1 justify-center">
          {navItems.slice(0, 4).map((item) => (
            ...
            >
              <span
                className={`text-[12px] tracking-[0.15em] uppercase transition-colors duration-300 font-bold whitespace-nowrap ${
                  activeSection === item.id ? 'text-[#FF0000]' : 'text-white/60'
                }`}
              >
                {item.label}
              </span>
        ...
        <div className="flex items-center gap-4 md:gap-8 flex-shrink-0">
          ...
          <div className="relative hidden md:block">
            <button 
              ...
              className="flex items-center gap-2 text-[11px] xl:text-sm tracking-[0.2em] hover:opacity-80 transition-opacity uppercase font-bold text-[#FF0000] whitespace-nowrap"
            >
            ...
            <AnimatePresence>
              {showLangMenu && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 10 }}
                  className="absolute top-full mt-4 right-0 bg-black border border-white/10 py-2 min-w-[80px]"
                >
                  {languages.map((lang) => (
                    <button
                      key={lang.id}
                      ...
                      className={`w-full px-4 py-2 text-left text-xs tracking-widest hover:bg-[#FF0000]/10 transition-colors uppercase ${
                        language === lang.id ? 'text-[#FF0000]' : 'text-white/60'
                      }`}
                    >
                      {lang.label}
                    </button>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 bg-black/75 backdrop-blur-md z-40 md:hidden flex flex-col justify-center items-center"
          >
              ...
              {navItems.map((item, index) => (
                <motion.button
                  ...
                  className={`text-4xl font-black tracking-tighter uppercase ${
                    activeSection === item.id ? 'text-[#FF0000]' : 'text-white'
                  }`}
                >
                  {item.label}
                </motion.button>
              ...
              {languages.map((lang) => (
                <button
                  ...
                  className={`text-sm tracking-widest font-bold uppercase ${
                    language === lang.id ? 'opacity-100' : 'opacity-40'
                  } text-[#FF0000]`}
                >
                  {lang.label}
                </button>
